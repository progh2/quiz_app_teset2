...
import 'package:quiz_app_test2/widget/widget_candidate.dart';
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  _buildCandidates(double width, Quiz quiz) {
    List<Widget> _children = [];
    for(int i = 0; i < quiz.candidates.length; i++){
      _children.add(
        CandWidget(
          index: i,
          ...
          tap:(){
            setState(() {
              for(int j =0; j < _answerState.length; j++){
                if(j == i){
                  _answerState[j] = true;
